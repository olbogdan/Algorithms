...
class Solution:
    def gridGame(self, grid: List[List[int]]) -> int:
        COL = len(grid[0])
        pref1 = grid[0].copy()  # create a shallow copy of the original list
        pref2 = grid[1].copy()  # for convenience
        ...
        res = float(inf)
        sumOrFirstRow = pref1[-1]
        for pivot in range(COL):
            left = pref2[pivot - 1] if pivot > 0 else 0
            right = sumOrFirstRow - pref1[pivot]
            maxRob2 = max(left, right)
            res = min(res, maxRob2)
        return res
