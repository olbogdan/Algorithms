...
class `1376 Time Needed to Inform All Employees` {
    fun numOfMinutes(n: Int, headID: Int, manager: IntArray, informTime: IntArray): Int {
        ...
        for (idx in manager.indices) {
            ...
            val parentId = manager[idx]
            adj.computeIfAbsent(parentId) { mutableListOf() }.add(idx)
        }

        fun dfs(parent: Int): Int {
            var res = 0

            val informPrice = informTime[parent]
            for (childIdx in adj.getOrDefault(parent, mutableListOf())) {
                res = maxOf(res, dfs(childIdx) + informPrice)
            }
